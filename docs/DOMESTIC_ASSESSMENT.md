...
정적 앱 기준선은 판정 대상이 아니다. `/coverage`에서 안전 일시정지 또는 종료된 Run을 선택하면 각 국내 기준 항목에 **취약 판정 작업**이 표시된다.

- `confirmed`: 같은 Run의 증적 중 해당 항목의 필수 유형과 일치하는 원본을 선택한다. 문서 기준 수행 확인, 검토자, 판정 근거가 모두 있어야 기록할 수 있다.
- `needs_review`: 후보 상태와 추가 검토 사유만 기록하며 증적을 확정 원장이나 DOCX에 연결하지 않는다.
- `not_vulnerable`, `not_applicable`: 상태와 사유만 기록한다. 양호·해당없음 증적은 생성·연결하지 않는다.
- 수동 첨부가 허용된 항목은 PNG, JPEG, TXT, LOG, JSON, XML, HAR 원본을 최대 16MB까지 받을 수 있다. 첨부는 항목 ID에 고정되며 다른 항목의 확정 증적으로 재사용할 수 없다.
- 단말 상태·프로세스·저장소·프록시처럼 자동 수집 유형이 필수인 항목은 일반 수동 첨부로 대체할 수 없다.

취약 확정 후 원장과 Run 증적을 다시 불러오며, 확정 항목이 하나 이상이면 **취약점만 DOCX**가 활성화된다.
