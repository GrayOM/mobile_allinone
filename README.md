...
`/data`의 **원본 데이터 보존** 화면에서는 프로젝트별 보존기간(1~3650일)과 Raw 열람 허용 여부를 관리한다. 기본 화면과 실시간 진단은 마스킹 데이터를 사용하고 Raw 패킷·원본 파일 인덱스는 프로젝트 정책을 활성화한 뒤 전용 화면에서 다시 확인해야 불러온다. 서버는 `Cache-Control: no-store`를 적용하고 파일 시스템 경로는 API에 노출하지 않는다. 보존기간이 지나도 자동 삭제하지 않으며 프로젝트 이름, 복구 불가 확인, 미리 본 정확한 Run ID가 서버의 최신 계산과 모두 일치할 때만 만료 Run의 DB 이력과 로컬 원본을 정리한다. 활성 Run과 앱 원본·정적 분석 기준선은 정리 대상이 아니다.
