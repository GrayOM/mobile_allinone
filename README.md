...
활성화하려면 고객사 또는 앱 소유자의 승인 참조, 승인된 테스트 범위, 명시적 진단 권한·테스트 환경·테스트 계정과 데이터 사용에 대한 세 가지 확인을 모두 입력해야 한다. 서버는 수신 시각과 함께 해당 값을 `approval_record` 증적으로 보존한다.

> 이 모드는 관찰·증적 수집 전용이다. 보안 통제를 자동으로 무력화하지 않고, 운영 계정·실제 고객 데이터에 접근하지 않으며, 승인 정보는 외부 AI 컨텍스트에 포함하지 않는다. 범위 밖 검증은 `manual_required`로 전환해 고객사와 솔루션 공급사의 별도 절차를 따른다.
